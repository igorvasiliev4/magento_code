            ...
            if ($isCheckSpecialFlag == 1) {       	
              ...
              $fields = array ();
               ...
               $dup = Mage::getModel('superdeals/dealz')->getCollection()
                ...
                if ($isCheckStatus == 1) {
                   $customArray= array('productName'=>$productName,'status'=>$status,'smallComplete'=>$smallComplete,'item'=>$item,'complete'=>$Complete,'orderNoWithEqual'=>$orderNoWithEqual,'ordrId'=>$ordrId,'orderTable'=>$orderTable,'order_id'=>$order_id);
...
public function getCheckStatus($customArray){
   ...
   $dupp = count ( $dup );
   $customUpdateArray = array('status'=>$status,'smallComplete'=>$smallComplete,'dupp'=>$dupp,'item'=>$item,'complete'=>$Complete,'connection'=>$connection,'orderNoWithEqual'=>$orderNoWithEqual,'ordrId'=>$ordrId,'orderTable'=>$orderTable,'order_id'=>$order_id);
