...
class Apptha_Marketplace_Block_Seller_Dashboard_Dashboard extends Mage_Core_Block_Template {
 ...
 public function mostViewed($id) {
  ...
  /**
   * get reports product collection
   */
  $products = Mage::getResourceModel ( 'reports/product_collection' )->addOrderedQty ()->addAttributeToSelect ( '*' )->setStoreId ( $storeId )->addStoreFilter ( $storeId )->addViewsCount ()->addAttributeToFilter ( 'visibility', array (
 ...
 public function getOrderCount($id,$status){
 	 ...
 	 */
 	if($status == 'completed'){
 	$statusArr = array('pending','processing','onhold'); 	
 	}elseif ($status == 'canceled'){
 	$statusArr = array('pending','processing','onhold','completed');
 	}else{
 	$statusArr = array('pending','processing','onhold','completed');
 	} 	
 	...
 	$statusCollection->addFieldToFilter( 'item_order_status', array( 'in' => $statusArr) );
